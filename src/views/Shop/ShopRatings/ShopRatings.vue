    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          ...
          > 全部<span class="count">{{shopRatings.length}}</span> </span>
          <span
            class="block positive"
            :class="{active:selectType===0}"
            @click="setselectType(0)"
          > 满意<span class="count">{{satisfiedCount}}</span> </span>
          <span
            class="block negative"
            :class="{active:selectType===1}"
            @click="setselectType(1)"
          > 不满意<span class="count">{{shopRatings.length-satisfiedCount}}</span> </span>
        </div>
        <div
          class="switch"
          :class="{on:onlyHasText}"
          @click="toggleOnlyHasText"
        >
...
export default {
  data () {
    return {
      selectType: 2,  // 选择评价类型：2全部 0满意 1不满意
      onlyHasText: false
    }
  },
  computed: {
    ...
    satisfiedCount () {
      return this.shopRatings.reduce((presize, rating) => presize + (rating.rateType === 0 ? 1 : 0), 0)
    },
    filterRatings () {
      let { selectType, onlyHasText, shopRatings } = this
      return shopRatings.filter((rating) => {
        ...
        // if (selectType === 2 || selectType === rateType) {
        //   if (!onlyHasText || text.length) {
        //     return true
        //   } else {
        //     return false
        //   }
  ...
  methods: {
    ...mapActions(['recShopRatings']),
    setselectType (type) {
      this.selectType = type
    },
    toggleOnlyHasText () {
      this.onlyHasText = !this.onlyHasText
    }
  },
  watch: {
